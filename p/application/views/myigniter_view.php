<div class="container-fluid">
	<div class="container-fluid my-5 pt-5">
		<div id="gogg" class="row g-4 pb-3 shadow-sm">
			<div class="col-sm-8">
				...
				<form action="<?= site_url("myigniter/selesai") ?>">
					<div class="row g-1">
						...
						<div class="col-sm-2">
							...
							<button style="background-color: #E04F62;" onclick="hapusSemua()" type="button" class="btn btn-danger btn_block"><i class="bi bi-trash3-fill"></i><small> (f9)</small></button>
						</div>
					</div>
				</form>
			</div>
			<div class="col-sm-4">
				...
				<div style="background-color: #FFFAF2;border-radius: 5px;" class="card p-4">
...
<script>
	$(function() {
		var availableTags = [
			<?php foreach ($cari->result() as $row) : ?> "<?= $row->id ?>",
			<?php endforeach ?>
		];
		$("#manual").autocomplete({
		...
		$('#bayare').keyup(function() {
			...
			$.get(site_url + '/myigniter/total', function(data) {
				...
				let ratusan = data.substr(-3);

				if (ratusan < 100) {
					let bulatam = (Math.floor(data / 1000)) * 1000;
					let bulatak = bulatam;
					let kembali = (Math.floor((bayare - bulatak)));
					if (kembali > 0 && bulatak > 0) {
					...
					} else if (parseInt(bayare) == parseInt(bulatak)) {
						if (bulatak > 0) {
	...
	function total() {
		...
		$.get(site_url + '/myigniter/total', function(data) {
			...
			let bulatam = (Math.floor(data / 1000)) * 1000;
			if (ratusan < 100) {
				let hasilBulat = bulatam;
				$("#total, .totalan").html(hasilBulat).priceFormat({
					prefix: 'Rp. ',
					thousandsSeparator: ',',
					centsLimit: 0
				});
			} else if (ratusan < 599) {
				let hasilBulat = bulatam + 500;
				$("#total, .totalan").html(hasilBulat).priceFormat({
					prefix: 'Rp. ',
					thousandsSeparator: ',',
					centsLimit: 0
